...
Inherits InternetSession
	...
	#tag Method, Flags = &h0
		Function GetFile(remoteName as String, local as FolderItem, bFailIfExists as Boolean = false) As string
		  // Sanity checks
		  if mFTPHandle = 0 then
		    return ( "Trying to set the current directory while not connected" )
		  ...
		  if local.Directory then
		    ...
		    if System.IsFunctionAvailable( "FtpGetFileW", "WinInet" ) then
		      success = FtpGetFileW( mFTPHandle, remoteName, local.NativePath, bFailIfExists, _
		    ...
		    if not success then
		      return  "Could not get the remote file"+chr(13)
		      
		    else
		      Return "File transfered"+chr(13)
		    end if
		  #endif
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function PutFile(f as FolderItem, remoteName as String = "") As string
		  // Sanity checks
		  if f.Directory then
		    return ( "Trying to put a directory by calling PutFile" )
		    
		  end if
		  
		  if mFTPHandle = 0 then
		    return ( "Trying to put a file while not connected" )
		  ...
		  dim flags as Integer = TransferType
		    ...
		    if not success then
		      return  "Could not upload the file" +chr(13)
		    else
		      return  "File transfered"+chr(13)
		    end if
		  #endif
	...
	#tag Method, Flags = &h1
		Protected Sub SetLocalDirectory(dir as FolderItem)
		    ...
		    if System.IsFunctionAvailable( "SetCurrentDirectoryW", "Kernel32" ) then
		      success = SetCurrentDirectoryW( dir.NativePath)
		    else
		      success = SetCurrentDirectoryA( dir.NativePath )
		    end if
